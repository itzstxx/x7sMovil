local RAW_URL = "https://raw.githubusercontent.com/itzstxx/x7sMovil/refs/heads/main/x7s.lua"
...
local function getSource(url)
    if game and type(game.HttpGet) == "function" then return game:HttpGet(url, true) end
...
local source do
    local ok, result = pcall(function() return getSource(RAW_URL) end)
    if not ok or type(result) ~= "string" or #result < 10 then
        error("[x7s Loader] No se pudo descargar x7sMovil.\n  → URL: "..RAW_URL.."\n  → Error: "..tostring(result), 2)
        return
    end
    source = result
end
...
if not fn then error("[x7s Loader] Error al compilar:\n  "..tostring(compileErr), 2); return end

local ok2, runtimeErr = pcall(fn)
if not ok2 then error("[x7s Loader] Error al ejecutar:\n  "..tostring(runtimeErr), 2); return end
